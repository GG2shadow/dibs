...
export default async function Home() {
  ...
  return (
    <div>
      ...
      <h2 className="mt-24 font-bold text-gray-700 text-3xl">What's popular</h2>
      ...
      <div className="mt-5 grid md:grid-cols-2 lg:grid-cols-3 min-gap-6">{projects.map((project) => (
        <Link href={`/projects/${project.slug}`} key={project._id}  className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
          {project.image && (
            <Image
              src={project.image}
              alt={project.name}
              width={750}
              height={300}
              className="object-cover border rounded-t-lg"
            />
          )}
